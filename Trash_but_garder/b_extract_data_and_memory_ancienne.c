...
int readwriteDocument(char* filename, struct Tree *trees, int size_org){
   ...
   FILE *file = fopen(filename, "r");
    ...
    FILE *file_out = fopen("trees_GE.csv", "w"); // Open the file in write mode
...
struct Tree* gettreearray(int size){
    ...
    for(int i = 0; i<size; i++){
        ...
        tree->ozone_forming_potential = 0.0;
        tree->PM10_deposition = 0.0;
        tree->O3_deposition = 0.0;
        tree->O3_annual_deposition = 0.0;
        tree->O3_annual_removal = 0.0;
        tree->O3_annual_removed_mass = 0.0;
        tree->O3_net_annual_uptake = 0.0;
        tree->PM10_annual_deposition = 0.0;
    }
    return trees;
}
